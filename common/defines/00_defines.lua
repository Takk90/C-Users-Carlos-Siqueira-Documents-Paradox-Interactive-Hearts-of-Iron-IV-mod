...
NDiplomacy = {
	...
	BASE_IMPROVE_RELATION_COST = 5,                -- Political power cost to initiate relation improvement --10
	...
	BASE_STAGE_COUP_DAILY_PP = 1.0,					-- Daily pp cost for staging a coup --0.5
	...
	FLEET_FEAR = 3,									-- Impact on troops on borders when deciding how willing a nation is to trade --1
	IC_TO_EQUIPMENT_COUP_RATIO = 0.2,				-- Ratio for calculating cost of staging coup --0.1
	...
	VOLUNTEERS_DIVISIONS_REQUIRED = 5,				-- This many divisons are required for the country to be able to send volunteers. --30
	...
	TENSION_VOLUNTEER_FORCE_DIVISION = 0.2,			-- Amount of tension generated for each sent division --0.5
	...
	BASE_CONDITIONAL_PEACE_WARESCORE_RATIO = 0.25, 	-- Warscore ratio needed for the losing side to able to surrender. --0.5
	...
	PEACE_SCORE_PER_PASS = 0.0,						-- When you pass in a peace deal you gain this percent of your original score --0.2
	...
	OPINION_PER_VOLUNTEER = 8,						-- Opinion bonus per one sent volunteer division --3
	MAX_OPINION_FROM_VOLUNTEERS = 40,				-- Opinion bonus per one sent volunteer division --30
...
NCountry = {
	...
	LOCAL_MANPOWER_ACCESSIBLE_NON_CORE_FACTOR = 0.00,  -- accessible recruitable factor base --0.02
	...
	DEFAULT_NATIONAL_UNITY = 0.65,					-- Default national unity if not scripted otherwise. --0.7
	MAX_NATIONAL_UNITY = 0.95,                      --0.90
...
NProduction = {
	...
	ANNEX_STOCKPILES_RATIO = 0.5,		-- How much stockpiled equipment will be transferred on annexation --1.0
	ANNEX_FIELD_EQUIPMENT_RATIO = 0.1,	-- How much equipment from deployed divisions will be transferred on annexation --0.25
	ANNEX_CONVOYS_RATIO = 0.05,			-- How many convoys will be transferred on annexation --0.15
...
NAI = {
	...
	RESEARCH_NAVAL_DOCTRINE_NEED_GAIN_FACTOR = 0.005, -- Multiplies value based on relative naval industry size / country size.
	...
	RESEARCH_BONUS_FACTOR = 10, 				-- To which extent AI should care about bonuses to research
	...
	AIR_CONTROLLED_TERRITORY_INTERCEPT_SCORE = 6,-- Interception score per controlled state in mission region			AIR_CONTROLLED_TERRITORY_INTERCEPT_SCORE = 6,	-- Interception score per controlled state in mission region
	AIR_ALLY_TERRITORY_INTERCEPT_SCORE = -400,	-- Interception score per ally controlled state in mission region			AIR_ALLY_TERRITORY_INTERCEPT_SCORE = 3,		-- Interception score per ally controlled state in mission region
	INTERCEPT_NAVAL_BOMBING_FACTOR = 35.0,		-- Factors the weight of intercepting naval bombers in sea regions where friendly ships are			INTERCEPT_NAVAL_BOMBING_FACTOR = 3.0,		-- Factors the weight of intercepting naval bombers in sea regions where friendly ships are
	AIR_CAS_SCORE_FACTOR = 25.0,				-- Factor applied to (friendly units) x (enemy units)) to get CAS score for region			AIR_CAS_SCORE_FACTOR = 0.05,				-- Factor applied to (friendly units) x (enemy units)) to get CAS score for region
	AIR_ENEMY_SUPERIORITY_FACTOR = 5.0,			-- How much enemy air superiority counts relative to own			AIR_ENEMY_SUPERIORITY_FACTOR = 2.0,			-- How much enemy air superiority counts relative to own
	AIR_SUPERIORITY_FACTOR = 2.5,				-- Factor for air superiority score			AIR_SUPERIORITY_FACTOR = 4.0,				-- Factor for air superiority score
	AIR_MIN_ASSIGN_SCORE = 10,					-- Minimum total score for region to be considered for air missions			AIR_MIN_ASSIGN_SCORE = 5,					-- Minimum total score for region to be considered for air missions
	AIR_MIN_REASSIGN_SCORE = 25,				-- Minimum total score for region to be considered for reassigning air missions			AIR_MIN_REASSIGN_SCORE = 150,				-- Minimum total score for region to be considered for reassigning air missions
	AIR_WING_SIZE_FACTOR = 1.0,					-- Impact on air score of assigning wings to mission			AIR_WING_SIZE_FACTOR = 0.5,					-- Impact on air score of assigning wings to mission
	AIR_MISSION_MIN_COVEREAGE = 0.2,			-- Between 0 and 1, limit for % coverage for air mission to be considered			AIR_MISSION_MIN_COVEREAGE = 0.50,			-- Between 0 and 1, limit for % coverage for air mission to be considered
	AIR_BASE_PRIORITY_DISTANCE_FACTOR = 25.0,	-- Weight of distance between region and airbase for airbase priority score			AIR_BASE_PRIORITY_DISTANCE_FACTOR = 1.5,	-- Weight of distance between region and airbase for airbase priority score
	MAX_VOLUNTEER_ARMY_FRACTION = 0.25,			-- Countries will not send more than their forces time this number to aid another country			MAX_VOLUNTEER_ARMY_FRACTION = 0.50,			-- Countries will not send more than their forces time this number to aid another country
	...
	VP_GARRISON_VALUE_FACTOR = 0.5,				-- Extent to which VP garrisons are prioritized, based on VP value and compared to other priority values.
	...
	PLAN_ATTACK_MIN_ORG_FACTOR_LOW = 0.85,		-- Minimum org % for a unit to actively attack an enemy unit when executing a plan			PLAN_ATTACK_MIN_ORG_FACTOR = 0.75,			-- Minimum org % for a unit to actively attack an enemy unit when executing a plan
	PLAN_ATTACK_MIN_STRENGTH_FACTOR_LOW = 0.4,	-- Minimum strength for a unit to actively attack an enemy unit when executing a plan			PLAN_ATTACK_MIN_STRENGTH_FACTOR = 0.3,		-- Minimum strength for a unit to actively attack an enemy unit when executing a plan
	...
	PLAN_ATTACK_MIN_STRENGTH_FACTOR_HIGH = 0.15,PLAN_FRONTUNIT_DISTANCE_FACTOR = 4,			-- Factor for candidate units distance to front positions.
	...
	STATE_COTROL_FOR_AREA_DEFENSE = 0.4,				-- To avoid AI sending area defense to area with very little foothold
	...
	NAVAL_MISSION_INVASION_BASE = 800,					-- Base score for region with naval invasion (modified dynamically by prioritizing orders)
