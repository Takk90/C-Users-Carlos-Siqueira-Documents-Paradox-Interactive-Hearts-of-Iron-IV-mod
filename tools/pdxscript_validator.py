                ...
                if (isInCommentBlock == False):
                    if (ignoreTillEndOfLine): # we are in a line comment, just continue going through the characters until we find an end of line
                        ...
                        if (c == '"' or c == "'"):
                            ...
                            print("ERROR: 4 space instead of TAB detected at {0} Line number: {1}".format(filepath,lineNumber))
                            bad_count_file += 1
...
def main():
    ...
    rootDir = "../../Modern_Day_4_HOI"
    if (os.path.exists("Modern_Day_4_HOI")):
        rootDir = "Modern_Day_4_HOI"

    for root, dirnames, filenames in os.walk(rootDir + '/' + args.module):
      for filename in fnmatch.filter(filenames, '*.txt'):
        pdx_list.append(os.path.join(root, filename))

    for filename in pdx_list:
        bad_count = bad_count + check_pdx_syntax(filename)

    print("------\nChecked {0} files\nErrors detected: {1}".format(len(pdx_list), bad_count))
    if (bad_count == 0):
        print("PDX Script validation PASSED")
    else:
        print("PDX Script validation FAILED")
